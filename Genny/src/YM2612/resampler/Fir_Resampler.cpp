...
*/
int Resampler::Fir_Resampler_avail()
{
  long count = 0;
  sample_t* in = buffer;
...
int Resampler::Fir_Resampler_initialize( int new_size )
{
  res       = 1;
  skip_bits = 0;
  imp_phase = 0;
...
void Resampler::Fir_Resampler_shutdown( void )
{
...
void Resampler::Fir_Resampler_clear()
{
  imp_phase = 0;
  if ( buffer_size )
  {
...
double Resampler::Fir_Resampler_time_ratio( double new_factor, double rolloff )
{
  int i, r;
...
/* Current ratio */
double Resampler::Fir_Resampler_ratio( void )
{
  return ratio;
}

/* Number of input samples that can be written */
int Resampler::Fir_Resampler_max_write( void )
{
  return buffer + buffer_size - write_pos;
}

/* Pointer to place to write input samples */
sample_t* Resampler::Fir_Resampler_buffer( void )
{
  return write_pos;
}

/* Number of input samples in buffer */
int Resampler::Fir_Resampler_written( void )
{
...
void Resampler::Fir_Resampler_write( long count )
{
  write_pos += count;
}

int Resampler::Fir_Resampler_read( sample_t* out, long count )
{
  sample_t* out_ = out;
  sample_t* in = buffer;
  sample_t* end_pos = write_pos;
 ...
 /* fixed (Eke_Eke) */
int Resampler::Fir_Resampler_input_needed( long output_count )
{
  long input_count = 0;
  unsigned long skip = skip_bits >> imp_phase;
  int remain = res - imp_phase;
...
int Resampler::Fir_Resampler_skip_input( long count )
{
